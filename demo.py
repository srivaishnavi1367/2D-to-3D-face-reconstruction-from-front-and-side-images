...
def get_cropping_transformation(image, face_detector, shape_predictor):
    ...
    if len(image.shape) == 2:  # If grayscale, reshape properly
        pass  # Grayscale is fine
    elif len(image.shape) == 3 and image.shape[-1] == 1:  # If single-channel grayscale
        image = image.squeeze(-1)  # Remove extra channel
    elif len(image.shape) == 3 and image.shape[-1] == 3:  # If RGB, keep as is
        pass
    else:
        raise ValueError("Unexpected image format! Ensure it is grayscale or RGB.")
    ...
    top = d.top()
    bottom = d.bottom()
    print(f"Face Bounding Box: Left={d.left()}, Right={d.right()}, Top={d.top()}, Bottom={d.bottom()}")
...
def get_cropped_image(img, cropping_tform):
    ...
    warped_image = warp(img, cropping_tform.inverse, output_shape=(256, 256))
...
def main():
    ...
    for images in face_imgs:
        ...
        cropped_image_front = cropped_image_front.astype(np.uint8)
        cropped_image_side = get_cropped_image(side_img, cropping_tform_side)
        cropped_image_side = cropped_image_side.astype(np.uint8)
